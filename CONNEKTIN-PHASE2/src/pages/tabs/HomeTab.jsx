      ...
      <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: "#fff", borderRadius: "20px 20px 0 0", zIndex: 801, maxHeight: "60vh", display: "flex", flexDirection: "column", fontFamily: "DM Sans, sans-serif" }}>
        ...
        <div style={{ overflowY: "auto", flex: 1, padding: "8px 0 24px" }}>
          {loading ? (
            ...
            return (
              <div key={u.uid} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 16px", cursor: "pointer" }} onClick={() => { onClose(); onUserTap(u.uid); }}>
                ...
                {badge && <span style={{ fontSize: 10, fontWeight: 700, padding: "2px 8px", borderRadius: 10, background: badge.bg, color: badge.color, border: `1px solid ${badge.border}`, flexShrink: 0 }}>{badge.label}</span>}
              </div>
            );
          })}
        </div>
  ...
  return (
    <>
      <div style={S.card}>
        <div style={S.cardHeader}>
          <div style={{ cursor: isOwn ? "default" : "pointer", position: "relative", flexShrink: 0 }} onClick={goToProfile}>
            <div style={isInCircle ? { borderRadius: "50%", outline: "2.5px solid #0D9488", outlineOffset: "2px", display: "inline-block" } : {}}>
              <Avatar uid={item.uid} photoURL={isOwn ? profile?.photoURL : item.photo} avatarId={isOwn ? (profile?.avatarId || profile?.avatar) : (item.avatarId || item.avatar)} displayName={isOwn ? profile?.displayName : item.name} plan={isOwn ? (profile?.plan || "trial") : (item.plan || "trial")} role={isOwn ? (profile?.role || "participant") : (item.role || "participant")} size={44} />
            </div>
            {isInCircle && <div style={S.bondBadge}><InfinityIcon size={9} color="#fff" /></div>}
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
              ...
              {isInCircle && <span style={S.circlePill}><InfinityIcon size={10} color="#0D9488" /><span>Circle</span></span>}
            </div>
        ...
        {item.image && <img src={item.image} alt="post" style={{ ...S.postImg, cursor: "zoom-in" }} onClick={() => setShowImageView(true)} />}

        {(likeCount > 0 || commentCount > 0) && (
          <div style={S.countsRow}>
            {likeCount > 0 && <span style={{ ...S.countText, cursor: "pointer" }} onClick={() => setShowLikes(true)}>👍 {likeCount}</span>}
            {commentCount > 0 && <span style={{ ...S.countText, marginLeft: "auto", cursor: "pointer" }} onClick={() => setShowComments(v => !v)}>{commentCount} comment{commentCount > 1 ? "s" : ""}</span>}
          </div>
        )}
        ...
        {isTrialUser ? (
          ...
          <div style={S.actionsRow}>
            <button style={{ ...S.actionBtn, color: liked ? "#0D9488" : "#666", fontWeight: liked ? 700 : 500 }} onClick={handleLike} disabled={liking}>
              <svg width="17" height="17" viewBox="0 0 24 24" fill={liked ? "#0D9488" : "none"} stroke="currentColor" strokeWidth="2"><path d="M14 9V5a3 3 0 0 0-3-3l-4 9v11h11.28a2 2 0 0 0 2-1.7l1.38-9a2 2 0 0 0-2-2.3H14z"/><path d="M7 22H4a2 2 0 0 1-2-2v-7a2 2 0 0 1 2-2h3"/></svg>
              Like
            </button>
        ...
        {!isTrialUser && showComments && (
          <div style={S.commentSection}>
            <div style={S.commentInputRow}>
              <Avatar uid={currentUser?.uid} photoURL={profile?.photoURL} avatarId={profile?.avatarId || profile?.avatar} displayName={profile?.displayName} plan={profile?.plan || "trial"} role={profile?.role || "participant"} size={32} />
              <div style={S.commentInputPill}>
                <input style={S.commentBox} placeholder="Add a comment…" value={commentText} onChange={e => setCommentText(e.target.value)} onKeyDown={e => e.key === "Enter" && handleComment()} />
                <button style={{ ...S.sendBtn, background: commentText.trim() ? "#0D9488" : "#E4E2DC" }} onClick={handleComment} disabled={!commentText.trim() || posting}>
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke={commentText.trim() ? "#fff" : "#aaa"} strokeWidth="2.5"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>
                </button>
              </div>
            </div>
            {comments.map(c => (
              <div key={c.id} style={S.commentItem}>
                <div style={{ cursor: c.uid !== currentUser?.uid ? "pointer" : "default" }} onClick={() => { if (c.uid && c.uid !== currentUser?.uid) setMiniCardUid(c.uid); }}>
                ...
                <div style={S.commentBubble}>
                  <div style={{ ...S.commentName, cursor: c.uid !== currentUser?.uid ? "pointer" : "default" }} onClick={() => { if (c.uid && c.uid !== currentUser?.uid) setMiniCardUid(c.uid); }}>{c.name}</div>
      ...
      {showImageView && item.image && <PostImageFullView src={item.image} onClose={() => setShowImageView(false)} />}
      {showLikes && <LikesSheet likedBy={item.likedBy || []} onClose={() => setShowLikes(false)} onUserTap={(uid) => setMiniCardUid(uid)} />}
    </>
  );
}
...
function ArticleCard({ item, navigate, onAuthorTap }) {
  ...
  return (
    <div style={{ ...S.card, padding: 0, cursor: "pointer" }} onClick={() => navigate(`/article/${item.id}`, { state: { from: "home" } })}>
      {item.coverImage ? (
        <div style={{ position: "relative" }}>
          <img src={item.coverImage} alt="cover" style={{ width: "100%", height: 160, objectFit: "cover", display: "block" }} />
          <div style={{ position: "absolute", bottom: 10, left: 12 }}><span style={{ background: lc, color: "#fff", fontSize: 10, fontWeight: 700, padding: "3px 8px", borderRadius: 20, textTransform: "uppercase", letterSpacing: "0.04em" }}>{item.level}</span></div>
          {item.readTime && <div style={{ position: "absolute", bottom: 10, right: 12, background: "rgba(0,0,0,0.5)", borderRadius: 10, padding: "3px 8px" }}><span style={{ color: "#fff", fontSize: 10, fontWeight: 500 }}>{item.readTime}</span></div>}
        </div>
      ) : <div style={{ height: 6, background: "#0D9488" }} />}
      <div style={{ padding: "12px 14px" }}>
        <div style={S.typeLabel}><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#0D9488" strokeWidth="2.5"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>Article</div>
        ...
        {item.summary && <div style={{ borderLeft: "3px solid #0D9488", background: "#F0FDFA", borderRadius: "0 8px 8px 0", padding: "8px 10px", margin: "8px 0" }}><p style={{ fontSize: 12, color: "#0F766E", lineHeight: 1.6, margin: 0, fontStyle: "italic" }}>{item.summary}</p></div>}
        <div style={{ display: "flex", alignItems: "center", marginTop: 10 }}>
          {!item.coverImage && <span style={{ fontSize: 11, fontWeight: 700, color: lc, background: lc + "18", borderRadius: 10, padding: "2px 10px", textTransform: "capitalize" }}>{item.level}</span>}
...
function JobCard({ item, navigate }) {
  ...
  return (
    <div style={{ ...S.card, padding: 0, cursor: "pointer", borderLeft: "3px solid #0EA5E9" }} onClick={() => navigate(`/jobs/${item.id}`)}>
...
export default function HomeTab() {
  ...
  // ── NEW: feed tab ─────────────────────────────────────────────
  const [feedTab, setFeedTab]         = useState("my"); // "all" | "my"
  // ─────────────────────────────────────────────────────────────

  const isAdmin = profile?.role === "admin";

  // Get user's industries — support both old single and new array
  const myIndustries = profile?.industries || (profile?.industry ? [profile.industry] : []);
  const primaryIndustry = myIndustries[0] || "medical_billing";
  const industryLabel = INDUSTRY_LABELS[primaryIndustry] || primaryIndustry;
  ...
  useEffect(() => {
    ...
    let postsQ;
    if (feedTab === "all" || isAdmin) {
      // All feed — no industry filter
      postsQ = query(collection(db, "posts"), limit(40));
    } else {
      // My feed — filter by all user industries
      if (myIndustries.length === 1) {
        postsQ = query(collection(db, "posts"), where("industry", "==", myIndustries[0]), limit(30));
      } else {
        // Firestore "in" supports up to 10 values
        postsQ = query(collection(db, "posts"), where("industry", "in", myIndustries.slice(0, 10)), limit(30));
      }
    }
    ...
    const unsub = onSnapshot(postsQ, async (postsSnap) => {
      ...
      let articlesSnap, jobsSnap;
      if (feedTab === "all" || isAdmin) {
        [articlesSnap, jobsSnap] = await Promise.all([
        ...
        const indFilter = myIndustries.slice(0, 10);
        [articlesSnap, jobsSnap] = await Promise.all([
          indFilter.length === 1
            ? getDocs(query(collection(db, "articles"), where("status", "==", "published"), where("industry", "==", indFilter[0]), limit(15)))
            : getDocs(query(collection(db, "articles"), where("status", "==", "published"), where("industry", "in", indFilter), limit(15))),
          indFilter.length === 1
            ? getDocs(query(collection(db, "jobs"), where("status", "==", "active"), where("industry", "==", indFilter[0]), limit(10)))
            : getDocs(query(collection(db, "jobs"), where("status", "==", "active"), where("industry", "in", indFilter), limit(10))),
        ]);
      }
    ...
    <div style={S.page}>
      ...
      <div style={S.searchWrap}>
        <div style={{ ...S.searchBox, borderColor: searchFocused ? "#0D9488" : "#E4E2DC" }}>
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#9CA3AF" strokeWidth="2" style={{ flexShrink: 0 }}><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
          <input style={S.searchInput} placeholder={`Search ConnektIn…`} value={searchQuery} onChange={e => setSearchQuery(e.target.value)} onFocus={() => setSearchFocused(true)} onBlur={() => setSearchFocused(false)} />
          {searchQuery && <button onClick={() => setSearchQuery("")} style={S.clearBtn}>✕</button>}
        </div>
      </div>

      {/* ── NEW: Feed Tab Switcher ── */}
      <div style={S.tabWrap}>
        <button
          style={{ ...S.tabBtn, color: feedTab === "my" ? "#0D9488" : "#9CA3AF", borderBottom: feedTab === "my" ? "2px solid #0D9488" : "2px solid transparent" }}
          onClick={() => setFeedTab("my")}
        >
          My Feed
          {myIndustries.length > 0 && (
            <span style={{ fontSize: 10, background: feedTab === "my" ? "#E6FAF8" : "#F3F2EF", color: feedTab === "my" ? "#0D9488" : "#9CA3AF", padding: "1px 6px", borderRadius: 10, marginLeft: 5, fontWeight: 600 }}>
              {myIndustries.length}
            </span>
          )}
        </button>
        <button
          style={{ ...S.tabBtn, color: feedTab === "all" ? "#0D9488" : "#9CA3AF", borderBottom: feedTab === "all" ? "2px solid #0D9488" : "2px solid transparent" }}
      ...
      <div style={S.composeBar}>
        <Avatar uid={profile?.uid} photoURL={profile?.photoURL} avatarId={profile?.avatarId || profile?.avatar} displayName={profile?.displayName} plan={profile?.plan || "trial"} role={profile?.role || "participant"} size={40} />
        <button style={S.composeBtn} onClick={() => navigate("/create-post")}>
          <span style={{ color: "#999", fontSize: 14 }}>Share something with {feedTab === "all" ? "ConnektIn" : industryLabel}…</span>
          <div style={S.composePlus}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.5"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg></div>
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div style={S.empty}>
          <svg width="52" height="52" viewBox="0 0 24 24" fill="none" stroke="#D1D5DB" strokeWidth="1.5"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
          <div style={{ color: "#9CA3AF", fontSize: 14, marginTop: 14, textAlign: "center" }}>
            {searchQuery ? "No results found." : feedTab === "my" ? "No posts in your industries yet. Try the All tab!" : "Your feed is loading. Check back soon."}
          </div>
          {!searchQuery && feedTab === "my" && (
            <button onClick={() => setFeedTab("all")} style={{ marginTop: 12, background: "#0D9488", color: "#fff", border: "none", borderRadius: 20, padding: "8px 20px", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "DM Sans, sans-serif" }}>
              View All Feed
            </button>
          )}
        </div>
...
const S = {
  ...
  // ── Tab switcher
  tabWrap:    { display: "flex", background: "#fff", borderBottom: "1px solid #E4E2DC", padding: "0 16px" },
  tabBtn:     { flex: 1, background: "none", border: "none", padding: "12px 0", fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: "DM Sans, sans-serif", display: "flex", alignItems: "center", justifyContent: "center", gap: 4, transition: "color 0.15s" },
  // ──
